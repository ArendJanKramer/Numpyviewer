...
namespace cnpy {
    ...
    struct NpyArray {
        ...
        std::string shape_str() {
            ...
            for (unsigned long i = 0; i < this->shape.size(); i++) {
                shapeStr.append(std::to_string(this->shape.at(i)));
    ...
    void parse_npy_header(char *buffer, size_t &word_size, char &data_type, std::vector<size_t> &shape,
                          bool &fortran_order);
    ...
    void npy_save(std::string fname, const T *data, const std::vector<size_t> shape, std::string mode = "w") {
        ...
        if (fp) {
            ...
            bool fortran_order;
            char data_type;
            parse_npy_header(fp, word_size, data_type, true_data_shape, fortran_order);
