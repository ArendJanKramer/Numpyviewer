...
static const float _viridis_data[256][3] = {{0.267004f, 0.004874f, 0.329415f},
                 ...
                 {0.993248f, 0.906157f, 0.143936}};

void getSeismicColormap(float value, float max, float min, uint8_t* result) {
    static float _seismic_data[256][3];
    static bool table_initialized = false;
    
    if (!table_initialized) {
        const float key_points[5][3] = {
            {0.0f, 0.0f, 0.3f},
            {0.0f, 0.0f, 1.0f},
            {1.0f, 1.0f, 1.0f},
            {1.0f, 0.0f, 0.0f},
        ...
        const float key_positions[5] = {0.0f, 0.25f, 0.5f, 0.75f, 1.0f};
        ...
        for (int i = 0; i < 256; i++) {
            ...
            int segment = 0;
            for (int j = 0; j < 4; j++) {
                if (pos >= key_positions[j] && pos <= key_positions[j+1]) {
                    segment = j;
                    break;
                }
            }
            
            float t = (pos - key_positions[segment]) / 
                     (key_positions[segment+1] - key_positions[segment]);
            
            _seismic_data[i][0] = key_points[segment][0] * (1.0f - t) + 
                                 key_points[segment+1][0] * t;
            _seismic_data[i][1] = key_points[segment][1] * (1.0f - t) + 
                                 key_points[segment+1][1] * t;
            _seismic_data[i][2] = key_points[segment][2] * (1.0f - t) + 
                                 key_points[segment+1][2] * t;
        }
    ...
    if (max - min <= 1e-6) {
        ...
        if (index_high > 255) index_high = 255;
        
        float weight_high = index_float - index_low;
        float weight_low = 1.0f - weight_high;
        
        float r = _seismic_data[index_low][0] * weight_low + 
                 _seismic_data[index_high][0] * weight_high;
        float g = _seismic_data[index_low][1] * weight_low + 
                 _seismic_data[index_high][1] * weight_high;
        float b = _seismic_data[index_low][2] * weight_low + 
                 _seismic_data[index_high][2] * weight_high;
        
        result[0] = static_cast<uint8_t>(r * 255);
        result[1] = static_cast<uint8_t>(g * 255);
        result[2] = static_cast<uint8_t>(b * 255);
    }
}

void getViridisColormap(float value, float max, float min, uint8_t* result) {
    if (max - min <= 1e-6) {
        result[0] = static_cast<uint8_t>(_viridis_data[127][0] * 255);
        result[1] = static_cast<uint8_t>(_viridis_data[127][1] * 255);
        result[2] = static_cast<uint8_t>(_viridis_data[127][2] * 255);
    } else {
        float normalized = (value - min) / (max - min);
        ...
        float r = _viridis_data[index_low][0] * weight_low + _viridis_data[index_high][0] * weight_high;
        float g = _viridis_data[index_low][1] * weight_low + _viridis_data[index_high][1] * weight_high;
        float b = _viridis_data[index_low][2] * weight_low + _viridis_data[index_high][2] * weight_high;
